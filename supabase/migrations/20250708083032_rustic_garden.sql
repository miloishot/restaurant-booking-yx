/*
  ...
  1. Analytics Functions
    ...
    - `get_daily_analytics` - Daily booking performance data  
    - `get_booking_trends` - Booking trends over time
    - `get_popular_dishes` - Most ordered menu items with revenue data
    - `get_revenue_analytics` - Complete revenue statistics from orders
    - `get_category_performance` - Menu category performance metrics

  2. Security
    - All functions use SECURITY DEFINER for proper access control
    - Functions validate restaurant ownership through existing RLS policies
...
BEGIN
  ...
  SELECT 
    ...
    ts.waitlist_count,
    ts.booking_count >= COALESCE(pt.threshold, 0)
  FROM time_slots ts
...
BEGIN
  ...
  WITH daily_stats AS (
    SELECT 
      ...
      AVG(b.party_size::numeric) as party_avg,
      CASE 
        WHEN COUNT(*) > 0 THEN COUNT(CASE WHEN b.was_on_waitlist THEN 1 END)::numeric / COUNT(*)::numeric * 100
        ELSE 0
      END as waitlist_pct
    FROM bookings b
    WHERE b.restaurant_id = p_restaurant_id
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(dr.rev_date, CURRENT_DATE),
    ROUND(COALESCE(dr.daily_rev, 0), 2)
  FROM revenue_stats rs
  LEFT JOIN (SELECT rev_date, daily_rev FROM daily_revenue WHERE rn = 1 LIMIT 1) dr ON true;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  SELECT 
    ...
    ROUND(
      CASE 
        WHEN tr.total_rev > 0 THEN (cs.revenue_sum / tr.total_rev) * 100
        ELSE 0
      END, 1
    )
