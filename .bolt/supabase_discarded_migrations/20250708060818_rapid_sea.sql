/*
  ...
  This migration fixes all RLS policies to ensure proper database access for:
  1. Table assignment during booking
  2. Booking status updates
  3. Customer management
  4. Waiting list operations
  5. Real-time updates and notifications

  ## Changes Made
  - Fixed booking policies for table assignment
  - Enhanced customer policies for booking operations
  - Improved waiting list policies
  - Added proper user profile policies
...
-- =============================================
-- STRIPE TABLES POLICIES (if needed)
-- =============================================

-- Ensure stripe tables have proper policies
CREATE POLICY IF NOT EXISTS "Users can view their own customer data"
  ON stripe_customers
  FOR SELECT
  TO authenticated
  USING (user_id = auth.uid() AND deleted_at IS NULL);

CREATE POLICY IF NOT EXISTS "Users can view their own subscription data"
...
CREATE POLICY IF NOT EXISTS "Users can view their own order data"
  ON stripe_orders
  FOR SELECT
  TO authenticated
  USING (
