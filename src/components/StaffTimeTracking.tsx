...
export function StaffTimeTracking({ restaurant }: StaffTimeTrackingProps) {
  ...
  const fetchTimeEntries = async () => {
    try {
      ...
      if (employeeIds.length > 0) {
        ...
        if (!empError) employeesData = empData || [];
      }

      const entriesWithEmployees = (data || []).map(entry => ({
        ...entry,
        employee: employeesData.find(emp => emp.employee_id === entry.temp_employee_id),
      }));
  ...
  const performPunchAction = async (employee: Employee, action: 'in' | 'out') => {
    try {
      ...
      if (action === 'in') {
        ...
        if (existingEntry) throw new Error('Employee is already punched in');
        ...
        if (entryError || !entry) throw new Error('No active punch in found for today');
  ...
  const handleDeleteEmployee = async (employee: Employee) => {
    try {
      ...
      setEmployeeForm({ name: '', role: 'staff', adminPassword: '' });
      fetchEmployees();
    } catch (error) {
  ...
  return (
    <Tab.Group>
      <div className="w-full max-w-7xl mx-auto">
        <Tab.List className="flex space-x-4 border-b mb-6 px-4">
          <Tab
            className={({ selected }) =>
              `py-3 px-6 -mb-[1.5px] font-medium transition-colors duration-200 border-b-2 ${
                selected
                            ...
                            <tr key={employee.employee_id} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm font-medium text-gray-900">
                                  {formatHours(calculateEmployeeHours(employee.employee_id || ''))}
                                </div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div>
                              ...
                              <div className="text-sm text-gray-500">
                                ID: {entry.employee?.employee_id?.substring(0, 8) || entry.temp_employee_id?.substring(0, 8)}...
                              </div>
                            </div>
                          </td>
